...
ap_uint<8> validate(HistCmd cmd) {
    ...
    if (ndata <= 0 || ndata > max_ndata)
        return (ap_uint<8>)HistError::INVALID_NDATA;
    if (nbins <= 0 || nbins > max_nbins)
        return (ap_uint<8>)HistError::INVALID_NBINS;
    if (!memmgr::is_word_aligned<32>(cmd.data_addr) ||
        ...
        return (ap_uint<8>)HistError::ADDRESS_ERROR;

    return (ap_uint<8>)HistError::NO_ERROR;
}
